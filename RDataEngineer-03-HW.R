...
# 場站名稱(中文)
sna2 <- sapply(results,FUN = function(x){return(x$sna)})
  
# 精度
lng2 <- sapply(results,FUN = function(x){return(x$lng)})
  
# 緯度
lat2 <- sapply(results,FUN = function(x){return(x$lat)})
  
# 總車位數
tot2 <- sapply(results,FUN = function(x){return(x$tot)})
  
# 目前車輛數目
sbi2 <- sapply(results,FUN = function(x){return(x$abi)})
